...
def play_game(snakePlayer, version):
    ...
    while True:
        ...
        ####Fitness Function:
        # -2 points to move away from food, 1 point moving toward. 35 points for
        #    getting the food.
        # Penalty must be worse for moving away, else will get stuck in loop
        if snakePlayer.length > previousLength:
            frameOfLastLengthChange == currentFrameNumber
            previousLength = snakePlayer.length
            FITNESS += 35 #Player got apple
            
        elif distance(applePos, snakePlayer.pos) < distanceToApple:
            if version != 3:
                FITNESS += 1
        else:
            if version == 1:
        ...
        if currentFrameNumber - frameOfLastLengthChange > BOARD_SIZE:
            ...
            return snakePlayer.length, FITNESS
            
        if result != None:
            finalLength = result[1]
            return finalLength, FITNESS
...
def train(numGens, batchSize, numBatchWinners,
    ...
    for genNum in range(numGens):
        ...
        for Ni, network in enumerate(batch):
            ...
            for i in range(NUM_TEST_GAMES):
                length, fitness = play_game(network, version)
    #print("LF: ", length, fitness)
                if i == 0:
    ...
    finalData = (winners, [SCORE_PROGRESSION, HIGH_SCORES, LOW_SCORES],
                [LENGTH_PROGRESSION, HIGH_LENGTH, LOW_LENGTH]
                )   
    if q is None:
